...
func TestPreviewGroupRoutingProfileTargetsReturnsNonSensitivePage(t *testing.T) {
	...
	assert.NotEmpty(t, response.Data.Items[0].RowKey)
	assert.NotEqual(t, response.Data.Items[0].RowKey, response.Data.Items[1].RowKey)
	assert.NotEmpty(t, response.Data.Items[0].TargetKey)
	assert.NotContains(t, recorder.Body.String(), `"issues":null`)
	assert.Contains(t, recorder.Body.String(), `"issues":[]`)
