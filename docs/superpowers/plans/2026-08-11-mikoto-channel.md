...
```go
require.Equal(t, 213, constant.ChannelTypeMikoto)
require.Equal(t, 214, constant.ChannelTypeDummy)
...
assert.True(t, isSeedanceTaskPlatform(constant.TaskPlatform("213")))
```
...
expect(CHANNEL_TYPE_OPTIONS).toContainEqual({ value: 213, label: 'Mikoto' })
...
Expected: FAIL，原因是 type 213 尚未注册。

- [x] **Step 4: 注册管理端渠道配置**

在 `constants.ts`、`channel-type-config.ts`、`channel-form.ts` 和 `channel-utils.ts` 中增加 type 213，配置如下：
...
将 213 加入 task-only、generic-test unsupported、managed default URL 和 pre-acceptance disabled 集合，不加入 model-fetchable 集合。
...
渠道默认禁用。发生上游协议不一致时，先禁用所有 type 213 渠道；代码回滚按提交逆序撤销 Mikoto E2E、管理端注册、后端注册、响应投影和请求方言。不要删除用户已有渠道或任务记录，保留失败测试和脱敏证据用于修正协议。
