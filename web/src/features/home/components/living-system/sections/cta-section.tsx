...
export function CtaSection({ isAuthenticated }: CtaSectionProps) {
  ...
  return (
    ...
    >
      <div
        className='border-border relative overflow-hidden rounded-3xl border px-10 py-20'
        style={{
          background:
            'radial-gradient(circle at 50% 0%, color-mix(in srgb, var(--color-violet) 20%, transparent), transparent 60%)',
        }}
      >
        <h2 className='relative text-[clamp(34px,4.5vw,56px)] leading-[1.05] font-bold tracking-[-0.035em]'>
          {t('Start routing in')}{' '}
