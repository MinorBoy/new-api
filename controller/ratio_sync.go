...
var pricingSyncFields = []string{
	...
	billing_setting.SeedanceTokenPriceField,
}
...
func valueMap(value any) map[string]any {
	...
	case map[string]types.DurationPrice:
		return lo.MapValues(typed, func(value types.DurationPrice, _ string) any { return value })
	case map[string]types.SeedanceTokenPrice:
		return lo.MapValues(typed, func(value types.SeedanceTokenPrice, _ string) any { return value })
	default:
		return nil
	}
}
...
func normalizeSyncValue(field string, value any) any {
	...
	if field == billing_setting.DurationPriceField {
		switch typed := value.(type) {
		case types.DurationPrice:
			return typed
		case *types.DurationPrice:
			if typed == nil {
				return nil
		...
		default:
			...
			var price types.DurationPrice
			if err := common.Unmarshal(data, &price); err == nil {
				return price
			}
		}
	}
	if field == billing_setting.SeedanceTokenPriceField {
		switch typed := value.(type) {
		case types.SeedanceTokenPrice:
			return typed
		case *types.SeedanceTokenPrice:
			if typed == nil {
				return nil
		...
		default:
			...
			var price types.SeedanceTokenPrice
			if err := common.Unmarshal(data, &price); err == nil {
				return price
			}
		}
	...
	for _, chn := range upstreams {
		...
		go func(chItem dto.UpstreamDTO) {
			...
			var pricingItems []struct {
				...
				SeedanceTokenPrice   *types.SeedanceTokenPrice `json:"seedance_token_price"`
			}
			...
			durationPriceMap := make(map[string]types.DurationPrice)
			seedanceTokenPriceMap := make(map[string]types.SeedanceTokenPrice)

			for _, item := range pricingItems {
				if item.ModelName == "" {
					continue
				}
				isDuration := item.BillingMode == billing_setting.BillingModePerDuration && item.DurationPrice != nil
				isSeedanceTokens := item.BillingMode == billing_setting.BillingModeSeedanceTokens && item.SeedanceTokenPrice != nil
				if isDuration {
					billingModeMap[item.ModelName] = billing_setting.BillingModePerDuration
					durationPriceMap[item.ModelName] = *item.DurationPrice
				} else if isSeedanceTokens {
					billingModeMap[item.ModelName] = billing_setting.BillingModeSeedanceTokens
					seedanceTokenPriceMap[item.ModelName] = *item.SeedanceTokenPrice
				...
				if !isDuration && !isSeedanceTokens {
					if item.QuotaType == 1 {
			...
			if len(durationPriceMap) > 0 {
				converted[billing_setting.DurationPriceField] = valueMap(durationPriceMap)
			}
			if len(seedanceTokenPriceMap) > 0 {
				converted[billing_setting.SeedanceTokenPriceField] = valueMap(seedanceTokenPriceMap)
			}
