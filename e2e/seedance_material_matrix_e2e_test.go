...
func TestSeedanceImportedMaterialMatrixFullFlowE2E(t *testing.T) {
	...
	for _, target := range targets {
		t.Run(target.CaseID, func(t *testing.T) {
			...
			require.Equal(t, int64(2), delta.LogCount, target.CaseID)
			require.Equal(t, 2, delta.ConsumeLogCount+delta.RefundLogCount, target.CaseID)
			require.Equal(t, 1, delta.ConsumeLogCount-delta.SettlementConsumeLogCount, target.CaseID)
			...
			require.Len(t, logs, 2, target.CaseID)
