...
**Tech Stack:** Go 1.22+, Gin, GORM v2, testify(`require` + `assert`),JSON 统一走 `common.*`。按秒和分辨率倍率计费(`OtherRatio: seconds`, `resolution`)。
...
9. **计费包含分辨率:** Dimensio `jimeng-video-seedance-2.0-vip` 的 1080p 单价是 720p 的 2.5 倍；EstimateBilling 必须返回有界的 `seconds` 和 `resolution` 倍率，并用 `PriceData.AddOtherRatio` 合并，不能只按秒计费。
...
✅ 按秒 + 分辨率倍率计费(OtherRatio: seconds/resolution)
...
```go
// EstimateBilling 返回有界时长和分辨率倍率。管理员配置的 ModelRatio 是
// Dimensio 720p 单秒基准价；jimeng-video-seedance-2.0-vip 的 1080p 为 2.5 倍。
func (a *TaskAdaptor) EstimateBilling(c *gin.Context, info *relaycommon.RelayInfo) map[string]float64 {
	...
	if arkReq.Duration == nil || *arkReq.Duration < 4 || *arkReq.Duration > relaycommon.MaxTaskDurationSeconds {
		return nil
	}
	ratios := map[string]float64{"seconds": float64(*arkReq.Duration)}
	resolution := strings.ToLower(strings.TrimSpace(arkReq.Resolution))
	if resolution == "1080p" {
...
- 720p 返回 `seconds=N` 和 `resolution=1`；1080p 返回 `seconds=N` 和 `resolution=2.5`；未知 resolution 返回 400。
- 映射后的目标模型必须属于文档列出的三个开放模型；fast-vip/mini 的 1080p 和未知模型均在访问上游前返回 400。
- `PriceData.AddOtherRatio` 拒绝的非正、NaN、Inf 值不会进入 adaptor 返回值。
- 查询响应即使出现未在文档中定义的 `duration` 扩展字段，也不得覆盖 `BillingContext.OtherRatios["seconds"]`。
- 查询错误 `1057/121101` 必须保持可重试，不得把任务标记失败或触发退款。
- ARK 与 OpenAI Video 查询都必须保留 `{code,message}` 错误 envelope 的 code/message。
- 成功任务保持提交阶段按请求 `duration × resolution` 计算的预扣额度。
- 失败任务进入公共退款路径，用户、渠道和 Token 三个账本均退回预扣额度。
- `ModelPrice` 仍按项目现有规则视为按次价格；建议为 Dimensio 目标模型配置代表 720p 单秒基准价的 `ModelRatio`。
...
将测试实际捕获的 ARK 请求、Dimensio 请求、Dimensio 成功/失败响应和 ARK 成功/失败响应写入 `docs/superpowers/reports/2026-07-19-dimensio-e2e-acceptance-report.md`。报告必须列出计费倍率、预扣 quota、成功保持预扣、失败三账本退款，以及未调用真实上游的边界。
...
2. 配模型映射：`doubao-seedance-2-0-260128` → `jimeng-video-seedance-2.0-vip`；在价格设置中把目标模型的 `ModelRatio` 配为 720p 单秒基准价。
...
- mock completed 响应只使用文档定义的 `status/progress/result.url`；成功后计费快照仍为提交请求的 `seconds` 与 `resolution`。
...
| 按秒计费和 1080p 分辨率倍率 | Task 4 Step 4、Step 11 |
...
- 计费路径覆盖请求校验 → 模型映射 → `OtherRatios` → 预扣 → 成功保持预扣/失败退款。
...
| Seedance 计费计划 | 本计划为 type 59 在提交阶段写入秒数/分辨率倍率；不修改 doubao 的官方价格矩阵，后续计费计划只复用公共结算链 |

**执行顺序:** 计划1(入口与原生查询)→ 本计划(Dimensio adaptor、task-only 注册、响应/计费接入)→ 计划2(Seedance/Doubao 官方价格矩阵)。本计划不等待后续计划才能运行；其 type 59 计费只依赖公共 `PriceData`/task 结算链。
