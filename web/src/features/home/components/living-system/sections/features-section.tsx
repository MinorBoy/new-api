...
const FEATURES: Feature[] = [
  ...
  },
  {
    ...
    descKey: 'OpenAI, Claude, Gemini, Responses APIs — all on one base URL.',
  },
  {
    num: '06',
    icon: Server,
  ...
  return (
    ...
    >
      <div className='mx-auto mb-16 max-w-[680px] text-center'>
        <span className='border-amber/30 bg-amber/10 text-amber inline-flex items-center gap-2 rounded-full border px-2.5 py-1 font-mono text-xs tracking-[0.2em] uppercase'>
          // {t('capabilities')}
        </span>
      ...
      <div className='grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3'>
        {FEATURES.map((f) => {
          ...
          return (
            <div
              key={f.num}
              className='bg-muted/30 border-border hover:bg-muted/50 relative cursor-default overflow-hidden rounded-2xl border p-7 transition duration-300 hover:-translate-y-[3px]'
              style={{ '--card-color': f.color } as React.CSSProperties}
            >
              <span className='text-muted-foreground/70 absolute top-6 right-6 font-mono text-xs font-medium'>
                {f.num}
              </span>
              <div
              ...
              <span
                className='absolute top-0 right-0 left-0 h-[3px] opacity-60 transition duration-300 group-hover:h-[5px] group-hover:opacity-100'
