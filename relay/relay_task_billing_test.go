...
func TestCostTaskSubmitUsesValidatedDurationOutsideUserDurationBilling(t *testing.T) {
	tests := []struct {
		...
		{name: "Paipu", channelType: constant.ChannelTypePaipu, body: `{"model":"client-video","content":[{"type":"text","text":"text"},{"type":"image_url","role":"reference_image","image_url":{"url":"https://8.8.8.8/ref.png"}}],"duration":8}`, upstreamModel: "imported-paipu-model", durationSeconds: 8, arkOfficialRoute: true},
