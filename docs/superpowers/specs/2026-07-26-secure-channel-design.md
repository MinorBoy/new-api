...
- 普通用户响应不得暴露 Key、上游任务 ID、渠道 ID 或分组内部信息。任务所有者可在标准 `content.video_url` 中收到播放所必需的完整结果 URL；日志、路由诊断和管理员审计不得记录该 URL 的敏感查询串。

## 7. 路由与计费

- 三个 Secure 渠道是独立候选，分别拥有渠道 ID、Key、成本、优先级、权重和健康状态。
- 路由目标同时绑定 `channel_id + upstream_model` 并声明分组能力，不允许请求跨用错误分组 Key。
- 现有路由约束补充向后兼容的 `input_modes` 和 `reference_minimums`：旧目标省略 `input_modes` 表示接受所有输入模式，省略 minimums 表示全 0。特价目标至少要求 1 张图片并只接受首帧或全能参考；海外目标接受文生、首帧、首尾帧和全能参考；企业目标接受文生、首帧和全能参考但不接受严格首尾帧。
- 策略保存校验使用每个目标支持的输入模式和最小素材数构造代表性 Ark facts，确认默认输出参数下至少存在一种可达输入；不能继续只用纯文生零素材 facts，否则合法的特价-only 策略无法保存。
