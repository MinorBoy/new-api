...
func TestChannelOtherSettingsValidateSecureVideoGroup(t *testing.T) {
	tests := []struct {
		...
		{name: "discount", isSecure: true, group: SecureVideoGroupDiscount},
		{name: "overseas", isSecure: true, group: SecureVideoGroupOverseas},
		{name: "enterprise", isSecure: true, group: SecureVideoGroupEnterprise},
		{name: "missing", isSecure: true, wantErr: "secure_video_group is required"},
		{name: "unknown", isSecure: true, group: "other", wantErr: "secure_video_group must be one of"},
		{name: "leak to other type", group: SecureVideoGroupDiscount, wantErr: "secure_video_group is only valid for Secure"},
		{name: "empty on other type"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			settings := ChannelOtherSettings{SecureVideoGroup: tt.group}
			err := settings.ValidateSecureVideoGroup(tt.isSecure)
			if tt.wantErr == "" {
				require.NoError(t, err)
				return
			}
