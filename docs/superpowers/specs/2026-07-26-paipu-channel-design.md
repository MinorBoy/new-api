...
本地派普资料只包含 `/v1/videos` 模型目录，不包含请求字段和响应样例；管理员已确认 Base URL 为 `https://api.paipu.net`。设计因此采用保守的 `/v1/videos` JSON 公共方言，并把真实上游请求与响应契约验收设为发布前置条件，而不是虚构未确认的能力。
...
资料确认这些模型使用 `POST /v1/videos`。默认 Base URL 为 `https://api.paipu.net`，管理端预填并允许管理员覆盖。
...
- Base URL 默认 `https://api.paipu.net`，允许管理员覆盖，保存时去除尾部 `/`；用户误填以 `/v1` 结尾时继续使用现有管理端警告。
...
- 失败任务走公共退款路径。
- 管理端新增 task-only 派普渠道，默认 Base URL 为 `https://api.paipu.net`，模型目录提供 24 个选项，API Key 使用普通单 Key 输入。
- 不增加派普专用分组字段、多 Key 结构或模型自动抓取。

## 8. 测试策略

- 单测覆盖 24 个模型目录、默认 Base URL 与覆盖值规范化、模型/分辨率一致性、文生请求翻译和所有拒绝边界。
...
- 不在缺少证据时宣称派普支持多模态。
- 不猜测除已确认 `https://api.paipu.net` 之外的备用域名或接口前缀。
