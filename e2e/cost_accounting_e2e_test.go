...
func TestCostAccountingSyncRetryAndLogPrivacyE2E(t *testing.T) {
	...
	assert.Equal(t, string(types.CostAttemptNotDispatched), detail.Attempts[0].Attempt.Status)
	...
	assert.Equal(t, string(types.CostProfitComplete), detail.Request.ProfitStatus)
	require.NotNil(t, detail.Request.BilledGrossProfitNanoUSD)
	assert.Equal(t, int64(-199_970_000), *detail.Request.BilledGrossProfitNanoUSD)
	...
	assert.Equal(t, int64(30_000), summary.RealizedRevenueNanoUSD)
	assert.Zero(t, summary.KnownIncompleteCostNanoUSD)
	assert.Equal(t, int64(1), summary.RetryAttemptCount)
	assert.Zero(t, summary.UnknownCostCount)
