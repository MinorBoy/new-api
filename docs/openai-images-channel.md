...
3. **渠道成本矩阵**：打开渠道的“模型成本”后，统一图像模型会显示 `1k/2k/4k × low/medium/high` 九宫格。每格填写单张上游成本，系统自动生成 `gen-*` 或 `edit-*` 成本变体；勾选“保存后激活”即可批量发布，空白的新格子会被忽略。历史版本仍保留在成本规则记录中。
4. **渠道 Image Profile**：渠道编辑页的 `image_profile` 定义档案版本、路径覆盖、上游模型映射和能力收窄。
5. **供应商成本规则**：成本规则按渠道、映射后的上游模型和 SKU 保存 `per_image` 成本，并通过草稿、校验、激活流程发布。
