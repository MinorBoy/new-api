...
func updatePricing() {
	...
	for model, groups := range modelGroupsMap {
		...
		} else if billingMode == billing_setting.BillingModeSeedanceTokens {
			if tokenPrice, ok := billing_setting.GetSeedanceTokenPrice(model); ok {
				pricing.BillingMode = billingMode
				pricing.SeedanceTokenPrice = &tokenPrice
				pricing.QuotaType = 1
			}
