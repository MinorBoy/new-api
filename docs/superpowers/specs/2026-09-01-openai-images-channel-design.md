...
- `cost_weighted`：先以最低已知成本为基准，按 `cost_tolerance_bps` 保留成本接近的候选，再用现有 `Weight + 10` 乘以成本倒数因子加权。成本越低获得的概率越高，但仍保留同容差池内的高价渠道。
...
- `cost_tolerance_bps` 为相对最低成本的容差，范围 `0..10000`，缺省为 `1000`（10%）。成本高于最低成本乘以 `1 + tolerance` 的渠道不进入混流池；未知成本始终不进入混流池。
...
### 6.3 `cost_weighted` 成本容差混流

`cost_weighted` 用于在低价渠道和高价回退之间做动态折中，不承诺固定的 80/20 等精确比例：

1. 仅保留成本已知、通过兼容性和最低毛利检查的候选。
2. 计算所有候选中的最低成本 `minCost`，保留 `cost <= minCost * (1 + cost_tolerance_bps / 10000)` 的候选。
3. 每个候选的有效权重为 `max(Weight + 10, 1) * minCost / cost`，使用有界整数运算；最低成本渠道因子最高，成本相同则完全复用 Weight 行为。
4. 重试排除失败渠道后重新计算最低成本、容差池和权重；不会沿用旧选择结果。

精确流量比例仍使用现有 `manual` 策略：将渠道设为同一 Priority，再按 Weight 配置目标比例。`cost_weighted` 的价值在于成本变化时自动倾斜，不能替代固定比例配置。
...
4. 添加图像 `lowest_cost` 选择器、`cost_weighted` 成本容差混流、重试重排和管理员诊断。
...
- `manual` 保持现有 Priority/Weight 行为；`lowest_cost` 将合格请求优先路由到预计成本最低的渠道，同价时使用 Priority/Weight；`cost_weighted` 在成本容差池中按成本倒数和现有 Weight 混流。
