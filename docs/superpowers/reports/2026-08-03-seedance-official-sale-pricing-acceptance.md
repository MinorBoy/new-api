...
| 页面 | 核验信号 |
| --- | --- |
| `/dashboard/overview` | 显示近 24 小时请求数 `110` 和消耗金额 |
| `/usage-logs/common` | 显示测试用户、测试分组、真实渠道和 Seedance 模型消费记录 |
| `/usage-logs/task` | 显示成功任务、入站路径 `/v1/video/generations`、任务 ID 和 `100%` |
| `/cost-accounting` | 显示官方售价收入、供应商成本、毛利润和毛利率明细 |
...
结果：持久化 Seed E2E 返回码为 `0`，生成 110 条成功任务、110 条使用日志、110 条成本请求和 110 次尝试；Go 全量测试通过；前端串行全量测试 `454 pass / 0 fail`；TypeScript 类型检查通过；生产构建通过。前端 `bun run lint` 未作为通过项报告，因为仓库当前存在与本轮无关的既有全量 lint 错误；本轮改动的定向测试、类型检查和构建均通过。
