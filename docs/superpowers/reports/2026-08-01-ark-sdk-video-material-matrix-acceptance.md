...
能力路由启用后，成本覆盖以具体路由目标的上游模型和成本 variant 为准，不再把同一渠道的客户端 canonical model 当作旧路由重复检查。没有任何路由目标或成本证据的 8yes 渠道保留真实历史类型 `1` 供审计，但以手动禁用状态写入，且启用能力数为 0。
